...
void setupVulkanPhysicalDevice(VulkanInstance &vulkanInstance, bool enableValidation)
{
	...
	if (enableValidation)
	{
		...
		setupVulkanDebug(vulkanInstance.instance, debugReportFlags, VK_NULL_HANDLE);
	}
	...
	VkBool32 validDepthFormat = vks::tools::getSupportedDepthFormat(physicalDevice, &vulkanInstance.depthFormat);
...
void prepareVulkan(VulkanInstance &vulkanInstance, uint32_t width, uint32_t height)
{
	...
	vulkanInstance.swapChain.initSurface(width, height);
	initializers::createVulkanCommandBuffers(vulkanInstance.drawCmdBuffers, vulkanInstance.swapChain);
	initializers::createWaitFences(vulkanInstance.waitFences, vulkanInstance.device);
	initializers::setupDepthStencil(vulkanInstance.vulkanDevice, vulkanInstance.depthFormat);
	initializers::setupRenderPass(logicalDevice, vulkanInstance.swapChain, vulkanInstance.depthFormat);
	initializers::createPipelineCache(logicalDevice, vulkanInstance.pipelineCache);
	initializers::setupFrameBuffer(logicalDevice, vulkanInstance.renderPass, width, height, vulkanInstance.swapChain.imageCount, vulkanInstance.frameBuffers);
}
