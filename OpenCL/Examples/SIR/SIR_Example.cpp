...
int main(int argc, char *argv[])
{
	...
	size_t seedBufferSize = count * sizeof(ulong);
	size_t trajectorySize = 3*(Nt+1)*sizeof(float);
	size_t trajectoryBufferSize = trajectorySize * count;
	
	assert(trajectoryBufferSize < cacheSize);

	std::array<float, 3 * (Nt + 1) * count> x_traj;
	...
	// status = clEnqueueReadImage(clInstance.commandQueue, x0Buffer, CL_FALSE, origin, region, 0, 0, x_traj.data(), 0, NULL, NULL);
	assert(status == CL_SUCCESS);
	...
	// for (int i = 0; i < Nt; i++)
	// {
	// 	std::cout << x_traj[3*i] << ", " << x_traj[3*i + 1] << ", " <<  x_traj[3*i + 2] << "\n";
	// }
	...
	outfile << "alpha,beta,N_pop,count,Nt,dt\n";
	outfile << alpha << "," << beta << "," << N_pop << "," << count << "," << Nt << "," << dt<< "\n";
	outfile.close();
