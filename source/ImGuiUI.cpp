...
void ImGuiUI::setupVisuals(float width, float height) {
  // to rgb #6D6D6D
  ...
  io.Fonts->AddFontFromFileTTF(fontPath.c_str(), 64.0f);
}
...
void ImGuiUI::destroyImGuiVulkanData() {
  ...
  vkDestroyDescriptorSetLayout(logicalDevice, ivData.descriptorSetLayout,
                               nullptr);
}

// Initialize all Vulkan resources used by the ui
void ImGuiUI::initializeResources(VkRenderPass &renderPass, VkQueue copyQueue) {
  ImGuiIO &io = ImGui::GetIO();
  // Create font texture
  ...
  VK_CHECK_RESULT(
      vkCreateImage(logicalDevice, &imageInfo, nullptr, &ivData.fontImage));
  ...
  VK_CHECK_RESULT(vkAllocateMemory(logicalDevice, &memAllocInfo, nullptr,
                                   &ivData.fontMemory));
  VK_CHECK_RESULT(
      vkBindImageMemory(logicalDevice, ivData.fontImage, ivData.fontMemory, 0));
  ...
  VK_CHECK_RESULT(
      vkCreateImageView(logicalDevice, &viewInfo, nullptr, &ivData.fontView));
  ...
  tools::setImageLayout(
      copyCmd, ivData.fontImage, VK_IMAGE_ASPECT_COLOR_BIT,
  ...
  vkCmdCopyBufferToImage(copyCmd, stagingBuffer.buffer, ivData.fontImage,
                         VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, 1,
                         &bufferCopyRegion);

  // Prepare for shader read
  tools::setImageLayout(copyCmd, ivData.fontImage, VK_IMAGE_ASPECT_COLOR_BIT,
  ...
  VK_CHECK_RESULT(
      vkCreateSampler(logicalDevice, &samplerInfo, nullptr, &ivData.sampler));
  ...
  VK_CHECK_RESULT(vkCreateDescriptorSetLayout(
      logicalDevice, &descriptorLayout, nullptr, &ivData.descriptorSetLayout));

  // Descriptor set
  VkDescriptorSetAllocateInfo allocInfo =
      initializers::descriptorSetAllocateInfo(ivData.descriptorPool,
                                              &ivData.descriptorSetLayout, 1);
  VK_CHECK_RESULT(vkAllocateDescriptorSets(logicalDevice, &allocInfo,
                                           &ivData.descriptorSet));
  VkDescriptorImageInfo fontDescriptor = initializers::descriptorImageInfo(
      ivData.sampler, ivData.fontView,
      VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
  std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
      initializers::writeDescriptorSet(
          ivData.descriptorSet, VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, 0,
          &fontDescriptor, 1)};
  ...
  VkPushConstantRange pushConstantRange = initializers::pushConstantRange(
      VK_SHADER_STAGE_VERTEX_BIT, sizeof(ivData.pushConstBlock), 0);
  VkPipelineLayoutCreateInfo pipelineLayoutCreateInfo =
      initializers::pipelineLayoutCreateInfo(&ivData.descriptorSetLayout, 1);
  pipelineLayoutCreateInfo.pushConstantRangeCount = 1;
  pipelineLayoutCreateInfo.pPushConstantRanges = &pushConstantRange;
  VK_CHECK_RESULT(vkCreatePipelineLayout(logicalDevice,
                                         &pipelineLayoutCreateInfo, nullptr,
                                         &ivData.pipelineLayout));
  ...
  VkGraphicsPipelineCreateInfo pipelineCreateInfo =
      initializers::pipelineCreateInfo(ivData.pipelineLayout, renderPass);
  ...
  std::vector<VkVertexInputBindingDescription> vertexInputBindings = {
      initializers::vertexInputBindingDescription(0, sizeof(ImDrawVert),
  ...
  shaderStages[0] = loadShader(logicalDevice, shadersPath + "ui.vert.spv",
                               VK_SHADER_STAGE_VERTEX_BIT);
  shaderStages[1] = loadShader(logicalDevice, shadersPath + "ui.frag.spv",
                               VK_SHADER_STAGE_FRAGMENT_BIT);

  VK_CHECK_RESULT(vkCreateGraphicsPipelines(logicalDevice, ivData.pipelineCache,
                                            1, &pipelineCreateInfo, nullptr,
                                            &ivData.pipeline));
}

// Starts a new imGui frame and sets up windows and ui elements
void ImGuiUI::newFrame(float frameTime, Camera &camera) {
...
void ImGuiUI::drawFrame(VkCommandBuffer commandBuffer) {
  ImGuiIO &io = ImGui::GetIO();

  vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                          ivData.pipelineLayout, 0, 1, &ivData.descriptorSet, 0,
                          nullptr);
  ...
  ivData.pushConstBlock.scale =
      glm::vec2(2.0f / io.DisplaySize.x, 2.0f / io.DisplaySize.y);
  ivData.pushConstBlock.translate = glm::vec2(-1.0f);
  vkCmdPushConstants(commandBuffer, ivData.pipelineLayout,
                     VK_SHADER_STAGE_VERTEX_BIT, 0,
                     sizeof(ivData.pushConstBlock), &ivData.pushConstBlock);
  ...
  if (imDrawData->CmdListsCount > 0) {
    ...
    vkCmdBindVertexBuffers(commandBuffer, 0, 1, &ivData.vertexBuffer.buffer,
                           offsets);
    vkCmdBindIndexBuffer(commandBuffer, ivData.indexBuffer.buffer, 0,
